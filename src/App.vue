<template>
	<v-app>
		<div class="gameWrapper">
			...
			<v-container text-xs-center style="display: flex;align-items: center;justify-content: center;">
						...
						<v-flex xs12 sm4 md4 lg4 xl4 class="bottomCard playerInfos">
							<v-card class="cardArea">
								<div class="playerAddress">
									<span class="playerTitle">{{$t("wrapper.address")}}</span><br>{{address}}
								</div>
								<div class="playerBalance">
									<span class="playerTitle">{{$t("wrapper.balance")}}</span><br>{{playerBalance}} NAS
								</div>
								<div class="playerBalance" style="color:#F44336;!important;font-weight:500;">
									<span class="playerTitle">{{$t("wrapper.netProfit")}}</span><br>{{playerProfit / 1e18 }} NAS
								</div>
								<div class="playerBtn">
									<v-btn color="info" @click="playRecord = !playRecord">{{$t("wrapper.playRecord")}}</v-btn>
									<v-btn color="info" @click="introduction = !introduction" style="margin-top: 25px;">{{$t("wrapper.gamePlayWay")}}</v-btn>
								</div>
							</v-card>
						</v-flex>
						<v-flex xs12 sm4 md4 lg4 xl4 class="bottomCard">
							<v-card class="playGame cardArea">
								<v-card-title primary-title>
									<div>
										<div class="JackpotInfos">
											{{$t("wrapper.bonusPool")}}
										</div>
										<h3 class="headline mb-0" style="font-size:2.6rem!important;font-weight:500;color:#F44336;">{{(jackpotBalance).toFixed(6)}} NAS</h3>
									</div>
								</v-card-title>
								<div class="poolInfos">
								...
								<div class="price" style="margin-top: 16px;font-size:1rem;font-weight:500;color:#F44336;">0.001 NAS / {{$t("wrapper.price")}}</div>
								<v-btn color="red" dark large @click="roll" style="background-color:#F76957;">{{$t("wrapper.playRoll")}} !</v-btn>
								<div class="gameTips">
									{{$t("wrapper.gameTips1")}}<br>{{$t("wrapper.gameTips2")}}
								</div>
							</v-card>
						</v-flex>
						<v-flex xs12 sm4 md4 lg4 xl4 class="bottomCard">
							<v-card class="betRecord cardArea">
								...
								<v-data-table flat :headers="rulesHeaders" :items="rulesArray" hide-actions class="elevation-0" style="color:#fb6853;">
									<template slot="items" slot-scope="props">
										<td style="font-weight:500;font-size:1.2rem;color:#fb6853;">{{ props.item.range }}</td>
										<td style="font-weight:500;font-size:1.2rem;color:#fb6853;">{{ props.item.bonus }}</td>
										<td style="font-weight:500;font-size:1.2rem;color:#fb6853;" v-if="props.item.fixed">{{ props.item.expect }} NAS</td>
										<td style="font-weight:500;font-size:1.2rem;color:#fb6853;" v-else>{{(jackpotBalance*props.item.expect).toFixed(6)}} NAS</td>
									</template>
								</v-data-table>
							</v-card>
						</v-flex>
					...
					<v-layout row wrap>
						<v-flex xs12 sm12 md12 lg12 xl12 class="rankCard">
							<v-card class="betRecord">
								...
								<v-data-table :headers="headers" :items="jackpotRecord" hide-actions class="elevation-1">
									<template slot="items" slot-scope="props">
										...
										<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.bonus / 1e18 }} NAS</td>
										<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.playerAddress }}</td>
									</template>
								</v-data-table>
							</v-card>
						</v-flex>
			...
			<div class="gameFooter">
				<div class="footerWrappe">
					...
					<div class="gamecontract">{{$t("wrapper.gameContract")}} : n1zT4n7MzWCxKWzpQJ5yVEqDEVt6xez6VKu</div>
					<div class="contactAuthor">
						<v-btn color="info" small @click="allRecord = !allRecord">{{$t("wrapper.allGameRecord")}}</v-btn>
					</div>
				</div>
			</div>
		</div>
		<v-dialog v-model="winTips" max-width="300px">
			<v-card class="checkingDialog">
				<v-card-title class="headline" style="font-weight:500;padding-top:0px;color:#F76957;">{{$t("winTips.congratulations")}} ! </v-card-title>
				<div style="font-size:1.2rem;font-weight:500;color:#F76957;">{{$t("winTips.got")}}
					<span style="font-size:1.4rem">{{resultInfos.Profit / 1e18}}
					</span> NAS !</div>
		...
		<v-dialog v-model="loseTips" max-width="300px">
			<v-card class="checkingDialog">
				<v-card-title class="headline" style="font-weight:500;padding-top:0px;">Oooops ! {{$t("loseTips.congratulations")}} ! </v-card-title>
		...
		<v-dialog v-model="checking" persistent max-width="300px">
			<v-card class="checkingDialog">
				<v-card-title class="headline" style="padding-top:0px;padding-bottom:28px;">{{$t("checking.checking")}}</v-card-title>
		...
		<v-dialog v-model="contact" max-width="300px">
			<v-card class="contactDialog">
				...
				<v-card-title class="headline" style="font-size:1.5rem!important;font-weight:500;">{{$t("contact.contactMe")}}!</v-card-title>
				<v-card-title class="headline" style="font-size:1.2rem!important;">TimSheng<br>{{$t("contact.des")}}!</v-card-title>
				<v-card-title class="headline" style="font-size:1.2rem!important;font-weight:500;">Mail: czihao1997@outlook.com</v-card-title>
			</v-card>
		</v-dialog>
		<v-dialog v-model="introduction" max-width="375px">
			<v-card class="introductionDialog">
				...
				<v-card-title class="headline" style="font-size: 1.1rem !important;font-weight: 400;">
					<span>
						<span style="font-size:1.3rem!important;font-weight:500!important;color:#F44336;"> Nebulas Lottery</span> {{$t("introduction.des")}}<br>
						<span style="font-size:1.3rem;font-weight:500;color:#F44336;">{{$t("introduction.play")}}!</span>
					</span>
				</v-card-title>
			</v-card>
		</v-dialog>
		<v-dialog v-model="playRecord" max-width="80%">
			<v-card>
				<v-data-table :headers="recordHeader" :items="playerRecord" class="elevation-0" style="text-align: center;">
					<template slot="items" slot-scope="props">
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.rollTime }}</td>
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.result }}</td>
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.bonus / 1e18 }} NAS</td>
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">
							<a target="_blank" :href="'https://explorer.nebulas.io/#/tx/'+props.item.rollHash">{{ props.item.rollHash }}</a>
						</td>
					</template>
					<template slot="pageText" slot-scope="props">
						Ligne {{ props.pageStart }}-{{ props.pageStop }} de {{ props.itemsLength }}
					</template>
				</v-data-table>
			</v-card>
		</v-dialog>
		<v-dialog v-model="allRecord">
			<v-card>
				<v-data-table :headers="headers2" :items="allRollRecord" class="elevation-0" style="text-align: center;">
					<template slot="items" slot-scope="props">
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.rollTime }}</td>
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.result }}</td>
						<td style="font-weight:500;font-size:1.1rem;color:#fb6853;">{{ props.item.bonus / 1e18 }} NAS</td>
				...
				setTimeout(function () {
					_num.animate({
						backgroundPositionY: numImgHeight * (60 - (num_arr[index]))
							...
							duration: 4000 + (3 - innerIndex) * 3000,
			...
			let options = {
				...
				successFunc: function (resp) {
					...
					for (let j = 0; j < self.allRollRecord.length; j++) {
						self.allRollRecord[j].rollTime = dayjs(dayjs(Number(self.allRollRecord[j].rollTime))).format("YYYY-MM-DD HH:mm:ss")
					}
					...
					for (let j = 0; j < self.jackpotRecord.length; j++) {
						self.jackpotRecord[j].rollTime = dayjs(dayjs(Number(self.jackpotRecord[j].rollTime))).format("YYYY-MM-DD HH:mm:ss")
					}
				}
			}
			nebRequest(options)
		...
		getPlayerRecord() {
			...
			let options = {
				...
				successFunc: function (resp) {
					...
					for (let j = 0; j < self.playerRecord.length; j++) {
						self.playerRecord[j].rollTime = dayjs(dayjs(Number(self.playerRecord[j].rollTime))).format("YYYY-MM-DD HH:mm:ss")
					}
					// self.jackpotRecord = JSON.parse(resp.result)
				}
			}
...
.application {
    font-family: 'Quicksand', 'PingFang SC', Arial, 'Hiragino Sans GB',
